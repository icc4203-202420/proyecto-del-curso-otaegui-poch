    ...
    if (currentUser?.id) {
      const fetchFriendsReviews = async () => {
        try {
          ...
          const response = await fetch(`http://192.168.1.13:3000/api/v1/friendships?user_id=${currentUser.id}`);
          if (!response.ok) {
          ...
          const reviewsPromises = friendIds.map(async (friendId) => {
            const reviewsResponse = await fetch(`http://192.168.1.13:3000/api/v1/reviews?user_id=${friendId}`);
          ...
          const userReviewsResponse = await fetch(`http://192.168.1.13:3000/api/v1/reviews?user_id=${currentUser.id}`);
